...
FastClassifier::ClassificationResult FastClassifier::classify(const std::string &transcript) const {
    ...
    if (best_match && best_score >= SIMILARITY_THRESHOLD) {
        ...
        if (result.action == "launch_application") {
            if (normalized_transcript.find("chrome") != std::string::npos || normalized_transcript.find("browser") !=
                std::string::npos) {
            ...
            } else if (normalized_transcript.find("down") != std::string::npos || normalized_transcript.find("decrease")
